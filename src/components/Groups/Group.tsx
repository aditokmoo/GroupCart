    ...
    return (
        <Link to={`/shopping-list/${data.id}`} className="flex h-48 cursor-pointer hover:opacity-80 w-full relative bg-white shadow-md">
            ...
            <div className="flex flex-col gap-2 p-4">
                ...
                <p className="text-xs absolute bottom-2 text-gray-500 flex items-center gap-1 italic"><IoInformationCircleOutline />{data.groupList.length === 0 ? `${data.createdBy} je kreirao grupu` : `${data.groupList[data.groupList.length - 1].addedBy} je dodao ${data.groupList[data.groupList.length - 1].item}`}</p>
